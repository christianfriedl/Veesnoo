LIBSOURCES=""
LIBSOURCES="$LIBSOURCES Logger.cc "
LIBSOURCES="$LIBSOURCES CursesException.cc CursesManager.cc CursesWindow.cc Rect.cc Widget.cc Label.cc "
LIBSOURCES="$LIBSOURCES Button.cc FocusableWidget.cc "
LIBSOURCES="$LIBSOURCES TextBox.cc Checkbox.cc MenuItem.cc "
LIBSOURCES="$LIBSOURCES Container.cc "
LIBSOURCES="$LIBSOURCES HorizontalMenu.cc VerticalMenu.cc FocusableContainer.cc ContainerFocusManaging.cc ContainerFocusManager.cc "
LIBSOURCES="$LIBSOURCES Window.cc PopupMenu.cc "
LIBSOURCES="$LIBSOURCES MainLoop.cc "
LIBSOURCES="$LIBSOURCES Screen.cc "
LIBSOURCES="$LIBSOURCES Event.cc BasicEvent.cc ChangeEvent.cc "
LIBOBJECTS="$LIBOBJECTS MessageBox.cc "
LIBOBJECTS="$LIBOBJECTS ConfirmBox.cc "
LIBOBJECTS="$LIBOBJECTS RadioButton.cc "
LIBOBJECTS="$LIBOBJECTS FocusStealer.cc "

LIBOBJECTS=""
LIBOBJECTS="$LIBOBJECTS Logger.o "
LIBOBJECTS="$LIBOBJECTS CursesException.o CursesManager.o CursesWindow.o Rect.o Widget.o Label.o "
LIBOBJECTS="$LIBOBJECTS Button.o FocusableWidget.o "
LIBOBJECTS="$LIBOBJECTS TextBox.o Checkbox.o MenuItem.o "
LIBOBJECTS="$LIBOBJECTS Container.o "
LIBOBJECTS="$LIBOBJECTS HorizontalMenu.o VerticalMenu.o FocusableContainer.o ContainerFocusManaging.o ContainerFocusManager.o "
LIBOBJECTS="$LIBOBJECTS Window.o PopupMenu.o "
LIBOBJECTS="$LIBOBJECTS MainLoop.o "
LIBOBJECTS="$LIBOBJECTS Screen.o "
LIBOBJECTS="$LIBOBJECTS Event.o BasicEvent.o ChangeEvent.o "
LIBOBJECTS="$LIBOBJECTS MessageBox.o "
LIBOBJECTS="$LIBOBJECTS ConfirmBox.o "
LIBOBJECTS="$LIBOBJECTS RadioButton.o "
LIBOBJECTS="$LIBOBJECTS FocusStealer.o "
