...
int main() {
    ...
    NVWidget *win = [[NVWidget alloc] initWithX: 38 Y: 13 Width: 30 Height: 20];
    NVLabel *l = [[NVLabel alloc] initWithX: 0 Y: 3];
    NVWidget *w = [[NVWidget alloc] initWithX: 0 Y: 0 Width: 20 Height: 1]];
    NVCheckbox *c = [[NVCheckbox alloc] initWithX: 0 Y: 1 Width: 3 Height: 1]];
    NVButton *b = [[NVButton alloc] initWithX: 0 Y: 2];
    NVMenuItem *m1 = [[NVMenuItem alloc] initWithText: @"m1" X: 0 Y: 0];
    NVMenuItem *m2 = [[NVMenuItem alloc] initWithText: @"m2" X: 0 Y: 0];
    NVHorizontalMenu *hm = [[NVHorizontalMenu alloc] initWithX: 0 Y: 4];
    [hm addWidget: m1];
    [hm addWidget: m2];
    [w addString: @"abcde"];
    getch();
